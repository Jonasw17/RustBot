...
FCM_CONFIG   = Path("rustplus.config.json")  # Created by FCM registration
...
class ServerManager:
    ...
    async def listen_for_pairings(self, callback: Callable):
        ...
        Requires rustplus.py.config.json (generated by `python pair.bat`).
        """
        if not FCM_CONFIG.exists():
            log.warning(
                "rustplus.config.json not found — run FCM registration first "
